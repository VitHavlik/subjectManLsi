...
let dtoIn = {
  ...
  test("studyProgramme/get - Error 2.2.1 ", async () => {
    expect.assertions(9)
    let dtoIn = {
        name: "Softwarový vývoj 2",
        description: "-",
    ...
    result = await TestHelper.executeGetCommand("studyProgramme/get", {id: result.id, unsupportedKey: "Ve středu bude pršet"})
  ...
  test("studyProgramme/list - HDS", async () => {
    ...
    result = await TestHelper.executeGetCommand("studyProgramme/list")
        ...
        expect(result.itemList[0]).toHaveProperty("degree")
        expect(result.itemList[0]).toHaveProperty("credits")
        expect(result.itemList[0]).toHaveProperty("languages")
        expect(result.itemList[0]).toHaveProperty("forms")
